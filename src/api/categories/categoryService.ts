...
export class CategoryService {
  ...
  async createCategory(categoryData: Category): Promise<ServiceResponse<Category | null>> {
    try {
  ...
  async getAllCategories(): Promise<ServiceResponse<Category[]>> {
    try {
      ...
      return ServiceResponse.failure(`Error fetching all categories: ${error}`, []);
    }
  }

  async getCategoryById(categoryId: string): Promise<ServiceResponse<Category | null>> {
    try {
      ...
      return ServiceResponse.failure(`Error fetching category by ID: ${error}`, null);
    }
  }
  ...
  async updateCategory(
    ...
    try {
      ...
      if (updatedCategory) {
        ...
        return ServiceResponse.failure('Category not found or update failed', null);
      }
    } catch (error) {
  ...
  async deleteCategory(categoryId: string): Promise<ServiceResponse<Category | null>> {
    try {
      const deletedCategory = await this.categoryRepository.delete(categoryId);
      if (deletedCategory) {
        return ServiceResponse.success('Category deleted successfully', deletedCategory);
      } else {
        return ServiceResponse.failure('Category not found or deletion failed', null);
      }
    } catch (error) {
